...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Deleting invoice from Zoho Books...");
    
    const deleteResponse = await fetch(
      `https://www.zohoapis.in/books/v3/invoices/${invoice_id}?organization_id=${organizationId}`,
      {
        method: "DELETE",
        headers: {
    ...
    console.error("Error in zoho-books-delete-invoice function:", error);
    return new Response(
      JSON.stringify({ 
