-- Fix the upsert_legalkart_case_data RPC function with correct column names
CREATE OR REPLACE FUNCTION public.upsert_legalkart_case_data(
  p_cnr_number text,
  p_firm_id uuid,
  p_case_id uuid DEFAULT NULL::uuid,
  p_case_data jsonb DEFAULT NULL::jsonb,
  p_documents jsonb DEFAULT NULL::jsonb,
  p_objections jsonb DEFAULT NULL::jsonb,
  p_orders jsonb DEFAULT NULL::jsonb,
  p_history jsonb DEFAULT NULL::jsonb,
  p_petitioners jsonb DEFAULT NULL::jsonb,
  p_respondents jsonb DEFAULT NULL::jsonb,
  p_ia_details jsonb DEFAULT NULL::jsonb
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_petitioners IS NOT NULL THEN
    ...
    -- Insert new petitioners (using petitioner_name, not name)
  ...
  IF p_respondents IS NOT NULL THEN
    ...
    -- Insert new respondents (using respondent_name, not name)
