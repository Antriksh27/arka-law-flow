...
-- 4. Add comprehensive audit logging for profile access
...
BEGIN
  ...
  INSERT INTO public.audit_logs (
    ...
    CASE 
      WHEN NEW.id != auth.uid() THEN 'medium'
      ELSE 'low'
    END
  );
...
-- 5. Enhance team member privilege escalation prevention
...
-- 6. Add function to check appointment rate limits
...
SELECT
  'sensitive_document_access' as metric,
  COUNT(*) as value,
  'Last 24 hours' as timeframe
...
CREATE INDEX IF NOT EXISTS idx_audit_logs_risk_level_timestamp ON public.audit_logs(risk_level, timestamp);
