-- Update the function to properly cast to enum type
...
BEGIN
  LOOP
    -- Update a batch of cases that are either low or high priority
    WITH batch AS (
      SELECT id 
      FROM cases 
    ...
    UPDATE cases 
    SET priority = target_priority::case_priority_enum,
        updated_at = NOW()
    FROM batch
