...
-- and consolidates conflicting RLS policies

-- Step 1: Drop all problematic SECURITY DEFINER views
...
-- Step 2: Recreate views as regular views (without SECURITY DEFINER)
-- This ensures proper RLS enforcement

-- Appointment details view with proper RLS enforcement
...
-- Case details view with comprehensive case information
...
-- Client statistics view for analytics
...
-- Firm statistics view for admin dashboard
...
-- Step 3: Fix profiles table RLS policy conflicts
-- Remove redundant and conflicting policies, implement clear access control
...
-- Policy 3: Users can view profiles within their firm
...
-- Policy 4: Admins can manage profiles within their firm
...
-- Step 4: Secure public appointments with authentication requirement
-- Add firm-based access control for public appointments
...
-- Create secure policies for public appointments
CREATE POLICY "public_appointments_authenticated_view" ON public.public_appointments
  FOR SELECT USING (
    -- Allow lawyers to view appointments for their booking pages
    lawyer_id IN (
      SELECT tm.user_id FROM team_members tm
      WHERE tm.firm_id IN (
        SELECT tm2.firm_id FROM team_members tm2 
        WHERE tm2.user_id = auth.uid()
      )
    )
    OR
    -- Allow the person who created the appointment to view it
...
CREATE POLICY "public_appointments_create_authenticated" ON public.public_appointments
  FOR INSERT WITH CHECK (
    ...
    auth.uid() IS NOT NULL
    AND
    -- Ensure lawyer exists and is in a valid firm
    lawyer_id IN (
      SELECT user_id FROM team_members 
      WHERE role IN ('lawyer', 'admin', 'partner', 'associate')
    )
  );

-- Step 5: Add comprehensive security monitoring
-- Log this security fix for audit trail
INSERT INTO public.audit_logs (
  entity_type, action, details
) VALUES (
  'security', 'critical_security_fixes_phase1_applied',
  jsonb_build_object(
    'timestamp', now(),
    'fixes_applied', jsonb_build_array(
      'removed_security_definer_views',
      'consolidated_profiles_rls_policies', 
      'secured_public_appointments',
    ...
    'security_improvement', 'eliminated_rls_bypass_vulnerabilities',
    'risk_level_reduced', 'critical_to_low'
  )
);