-- Phase 3: Data Protection Enhancement Security Migration (Fixed)

-- 1. Enable pgcrypto extension first
CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- 2. Create encryption functions for PII data
...
AS $$
  SELECT encode(pgp_sym_encrypt(data, 'hru_legal_encryption_key_2024'), 'base64');
$$;
...
SET search_path = 'public'
AS $$
  SELECT pgp_sym_decrypt(decode(encrypted_data, 'base64'), 'hru_legal_encryption_key_2024');
$$;

-- 3. Add encryption columns for sensitive PII data
...
-- 4. Create secure password generation table for temporary storage
...
-- 5. Enhanced audit log security with integrity checks
...
-- 6. Enhanced document access monitoring
