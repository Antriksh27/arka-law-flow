...
-- Remove ALL existing policies and implement clean access control
...
CREATE POLICY "public_appointments_create_authenticated" ON public.public_appointments
  FOR INSERT WITH CHECK (
    ...
    AND lawyer_id IN (
      ...
      WHERE role IN ('lawyer', 'admin', 'associate')
    )
  );

-- Step 5: Add comprehensive rate limiting for public endpoints
CREATE OR REPLACE FUNCTION public.enhanced_appointment_rate_limit(p_user_id uuid DEFAULT auth.uid())
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  daily_count INTEGER;
  max_hourly INTEGER := 5;  -- Stricter limit for public endpoints
  ...
  IF request_count >= max_hourly OR daily_count >= max_daily THEN
    INSERT INTO public.audit_logs (
      ...
      jsonb_build_object(
        ...
        'max_daily', max_daily,
        'user_role', COALESCE(user_role, 'anonymous'),
        'risk_level', 'medium'
      )
    );
    RETURN FALSE;
...
INSERT INTO public.audit_logs (
  ...
  jsonb_build_object(
    ...
    'fixes_applied', jsonb_build_array(
      ...
      'eliminated_rls_bypass_vulnerabilities'
    ),
    ...
    'security_improvement', 'database_security_vulnerabilities_fixed',
