...
import { Plus, Upload, Link as LinkIcon, CheckCircle } from 'lucide-react';
  ...
  return <>
      ...
      <PullToRefresh onRefresh={handleRefresh}>
        <div className="max-w-7xl mx-auto p-3 sm:p-6 space-y-4 sm:space-y-6 pb-24 sm:pb-6">
          ...
          {isMobile && (isLoadingStats ? <Skeleton className="h-32 rounded-2xl" /> : caseStats ? <Card className="p-4 bg-gradient-to-br from-primary/20 via-blue-100 to-accent/20 border-border shadow-md">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="flex flex-col items-center p-3 bg-white/80 rounded-xl">
                    ...
                    {caseStats.urgent === 0 ? <div className="text-xs text-green-600 flex items-center gap-1">
                        <CheckCircle className="w-3 h-3" />
                        All clear!
                      </div> : <div className="text-xs text-muted-foreground">Urgent Today</div>}
                  </div>
          ...
          {/* Mobile Tabs */}
          {isMobile && <Tabs value={casesTab} onValueChange={value => setCasesTab(value as 'all' | 'in_court' | 'disposed' | 'my')} className="w-full">
          ...
          {!isMobile && <Tabs value={casesTab} onValueChange={value => setCasesTab(value as 'all' | 'in_court' | 'disposed' | 'my')} className="w-full">
              ...
              <TabsContent value="all" className="space-y-6 mt-0">
                ...
                <CasesTable searchQuery={searchQuery} statusFilter="all" typeFilter={typeFilter} assignedFilter={assignedFilter} showOnlyMyCases={false} />
              </TabsContent>

              <TabsContent value="in_court" className="space-y-6 mt-0">
                <CasesFilters searchQuery={searchQuery} onSearchChange={setSearchQuery} statusFilter={statusFilter} onStatusChange={setStatusFilter} typeFilter={typeFilter} onTypeChange={setTypeFilter} assignedFilter={assignedFilter} onAssignedChange={setAssignedFilter} statusOptions={statusOptions} />
                <CasesTable searchQuery={searchQuery} statusFilter="in_court" typeFilter={typeFilter} assignedFilter={assignedFilter} showOnlyMyCases={false} />
              </TabsContent>

              <TabsContent value="disposed" className="space-y-6 mt-0">
                <CasesFilters searchQuery={searchQuery} onSearchChange={setSearchQuery} statusFilter={statusFilter} onStatusChange={setStatusFilter} typeFilter={typeFilter} onTypeChange={setTypeFilter} assignedFilter={assignedFilter} onAssignedChange={setAssignedFilter} statusOptions={statusOptions} />
                <CasesTable searchQuery={searchQuery} statusFilter="disposed" typeFilter={typeFilter} assignedFilter={assignedFilter} showOnlyMyCases={false} />
              </TabsContent>

              <TabsContent value="my" className="space-y-6 mt-0">
                <CasesFilters searchQuery={searchQuery} onSearchChange={setSearchQuery} statusFilter={statusFilter} onStatusChange={setStatusFilter} typeFilter={typeFilter} onTypeChange={setTypeFilter} assignedFilter={assignedFilter} onAssignedChange={setAssignedFilter} statusOptions={statusOptions} />
                <CasesTable searchQuery={searchQuery} statusFilter="all" typeFilter={typeFilter} assignedFilter={assignedFilter} showOnlyMyCases={true} />
              </TabsContent>
            </Tabs>}

          {/* Mobile: Show grid based on active tab */}
          {isMobile && <CasesGrid searchQuery={searchQuery} statusFilter={casesTab === 'all' ? 'all' : casesTab === 'in_court' ? 'in_court' : casesTab === 'disposed' ? 'disposed' : 'all'} typeFilter={typeFilter} assignedFilter={assignedFilter} showOnlyMyCases={casesTab === 'my'} />}
