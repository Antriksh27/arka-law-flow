...
export const normalizeEcourtsCnr = (cnr?: string | null): string =>
  (cnr ?? '').toUpperCase().replace(/[^A-Z0-9]/g, '');

export const resolveEcourtsSearchType = ({
  cnr,
  courtType,
  ...
  fallback?: ECourtsSearchType;
}): ECourtsSearchType => {
  const normalizedCnr = normalizeEcourtsCnr(cnr);
