...
export const ConvertToClientDialog = ({
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('team_members').select('id, user_id, profiles:user_id(id, full_name)').eq('firm_id', firmId).in('role', ['admin', 'lawyer']).order('profiles(full_name)');
  ...
  const convertToClientMutation = useMutation({
    mutationFn: async () => {
      ...
      let existingClientQuery = supabase.from('clients').select('id, full_name').eq('firm_id', firmId);
      if (contact.email) {
      ...
      if (contact.state_id) {
        const {
          data: stateData
        } = await supabase.from('states').select('name').eq('id', contact.state_id).single();
        stateName = stateData?.name || null;
      }
      if (contact.district_id) {
        const {
          data: districtData
        } = await supabase.from('districts').select('name').eq('id', contact.district_id).single();
      ...
      // Combine notes
      const combinedNotes = [contact.notes, contact.visit_purpose, additionalNotes].filter(Boolean).join('\n\n');
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="bg-muted/50 rounded-lg p-4 space-y-3">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                {contact.organization && <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <div className="space-y-2">
              {contact.email && <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  <Mail className="h-3 w-3" />
                  {contact.email}
                </div>}
              {contact.phone && <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <div className="space-y-2">
            ...
            <Select value={assignedLawyerId} onValueChange={setAssignedLawyerId}>
              ...
              <SelectContent>
                {lawyers.map((lawyer: any) => <SelectItem key={lawyer.user_id} value={lawyer.user_id}>
          ...
          {/* Corporate Fields */}
          {clientType === 'Corporate' && <div className="space-y-4 p-4 border rounded-lg bg-white/30">
              <h4 className="font-medium text-sm">Corporate Details</h4>
              
              <div className="space-y-2">
                <Label htmlFor="designation">Designation</Label>
                <Input id="designation" value={designation} onChange={e => setDesignation(e.target.value)} placeholder="e.g., Managing Director" />
              </div>

              <div className="space-y-2">
                <Label htmlFor="company-address">Company Address</Label>
                <Textarea id="company-address" value={companyAddress} onChange={e => setCompanyAddress(e.target.value)} placeholder="Company address" rows={2} />
              </div>

              <div className="space-y-2">
                <Label htmlFor="company-phone">Company Phone</Label>
                <Input id="company-phone" value={companyPhone} onChange={e => setCompanyPhone(e.target.value)} placeholder="Company phone number" />
              </div>

              <div className="space-y-2">
                <Label htmlFor="company-email">Company Email</Label>
                <Input id="company-email" type="email" value={companyEmail} onChange={e => setCompanyEmail(e.target.value)} placeholder="company@example.com" />
              </div>
            </div>}
          ...
          <div className="space-y-2">
            ...
            <Textarea id="additional-notes" value={additionalNotes} onChange={e => setAdditionalNotes(e.target.value)} placeholder="Any additional information about this client..." rows={3} />
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          <Button type="button" onClick={() => convertToClientMutation.mutate()} disabled={convertToClientMutation.isPending}>
