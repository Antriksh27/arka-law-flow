...
const WorkspaceDashboard = () => {
  ...
  const upcomingEvents = (data?.timelineEvents || []).filter(e => new Date(e.date) >= new Date()).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()).slice(0, 3).map(e => ({
  ...
  const weekCalendarEvents = Array.from({
    length: 7
  }, (_, i) => {
    const day = addDays(startOfWeek(new Date(), {
      weekStartsOn: 0
    }), i);
  ...
  return <>
      ...
      <PullToRefresh onRefresh={handleRefresh}>
        <div className="bg-gray-50 overflow-x-hidden min-h-screen">
          {/* Desktop Header */}
          {!isMobile && <div className="bg-white border-b border-gray-200 px-4 py-3 md:px-8 md:py-6">
          ...
          <div className="max-w-[1800px] mx-auto px-3 py-3 md:px-8 md:py-6 w-full">
            ...
            {isMobile ? <div className="space-y-4">
                ...
                {/* At a Glance Metrics */}
                <MetricsStrip activeCases={activeCases} todayEvents={todayEvents} pendingTasks={pendingTasks} weekEvents={weekEvents} isLoading={isLoading} />

                {/* My Active Cases (Top 3) */}
                <CompactCases cases={(data?.caseHighlights || []).slice(0, 3).map(c => ({
              id: c.id,
              title: c.case_title,
              ...
              next_hearing_date: c.next_hearing_date
                ...
                {/* Tasks & Notes Grid */}
                <TasksNotesGrid tasks={(data?.myTasks || []).filter(t => t.status !== 'completed').slice(0, 2)} notes={(data?.myNotes || []).slice(0, 2).map(n => ({
              id: n.id,
              ...
              color: n.color
                ...
                {/* Quick Actions Bar */}
                <QuickActionsBar onNewCase={() => setShowCaseDialog(true)} onSchedule={() => setShowAppointmentDialog(true)} onAddTask={() => setShowTaskDialog(true)} onUpload={() => setShowDocumentDialog(true)} />

                {/* Collapsible Updates Section */}
                <UpdatesSection documents={(data?.recentDocuments || []).slice(0, 2).map(d => ({
              id: d.id,
            ...
            }))} chats={[]} isLoading={isLoading} />
              </div> : (/* Desktop: 3-Column Grid */
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6">
                {/* Left Column - Today's Flow */}
                <div className="lg:col-span-3 order-2 lg:order-none">
                  <TodayFlow events={data?.timelineEvents || []} isLoading={isLoading} />
                </div>

                {/* Center Column - My Workspace */}
                <div className="lg:col-span-6 order-3 lg:order-none">
                  <MyActiveCases cases={(data?.caseHighlights || []).map(c => ({
                ...c,
                client_name: (c as any).client?.full_name || 'Unknown Client'
              }))} isLoading={isLoading} />

                  <PinnedNotes notes={(data?.myNotes || []).map(n => ({
                id: n.id,
                title: n.title || '',
                content: n.content || '',
                ...
                color: n.color || undefined
                  ...
                  <MyTasks tasks={data?.myTasks || []} isLoading={isLoading} />
                  
                  <RecentDocuments documents={(data?.recentDocuments || []).map(d => ({
                id: d.id,
                ...
                tasks: data?.metrics?.tasks || 0
                  ...
                  <LegalUpdates news={data?.legalNews || []} isLoading={isLoading} />
                </div>
              </div>)}
          </div>
        </div>
