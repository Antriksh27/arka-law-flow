          ...
          <CommandList className="max-h-[300px]">
            <CommandEmpty className="text-gray-500 py-6">
              <div className="text-center">
                <p>No cases found.</p>
                {clientId && (
                  <p className="text-sm mt-1">Select a different client to see more cases.</p>
                )}
              </div>
            </CommandEmpty>
            ...
            {cases.length > 0 && (
              <CommandGroup heading="Cases" className="text-gray-700">
                {cases.map((caseItem) => (
                  <CommandItem
                    key={caseItem.id}
                    value={`${caseItem.case_title} ${caseItem.case_number || ''}`}
                    onSelect={() => {
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {caseItem.case_number && (
                        <div className="text-xs text-gray-500 mt-0.5">{caseItem.case_number}</div>
                      )}
                    </div>
                    <Check
                      className={cn(
