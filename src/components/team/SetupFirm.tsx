...
export function SetupFirm({ user }: SetupFirmProps) {
  ...
  const handleCreateFirm = () => {
    setIsCreateFirmDialogOpen(true);
  }
    ...
    <>
      <div className="container max-w-2xl flex h-full w-full flex-col items-center justify-center gap-6 py-12 bg-slate-50">
        <Card className="w-full shadow-lg">
          <CardHeader>
            <CardTitle className="text-2xl">Join or Create a Firm</CardTitle>
            <p className="text-muted-foreground pt-2">
              Your account isn't associated with a law firm yet. To get started, join an existing firm or create a new one.
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            {isLoading ? (
              ...
              <div className="text-center py-8 px-4 border-dashed border-2 rounded-lg">
                  <p className="text-muted-foreground">No firms found. You'll need to create one to continue.</p>
              </div>
            )}
          </CardContent>
          <CardFooter className="border-t pt-6">
              <Button variant="outline" onClick={handleCreateFirm}>Create a New Firm</Button>
          </CardFooter>
        </Card>
      </div>
      <CreateFirmDialog 
        isOpen={isCreateFirmDialogOpen} 
        onOpenChange={setIsCreateFirmDialogOpen} 
      />
    </>
  );
}
