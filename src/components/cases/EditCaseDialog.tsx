...
export const EditCaseDialog: React.FC<EditCaseDialogProps> = ({
  ...
  const updateCaseMutation = useMutation({
    mutationFn: async (data: any) => {
      const processedData = {
        ...
        by_against: (data.by_against === '' || data.by_against === 'not_specified') ? null : data.by_against,
        acts,
        sections,
        orders,
    ...
    onSuccess: async () => {
      // Invalidate all case-related queries to ensure fresh data
  ...
  const handleManualSubmit = () => {
    ...
    if (!formData.client_id) {
      toast.error("Error: Client ID is missing");
      return;
    }
      ...
      <div className="px-6 py-4 border-t border-slate-100 bg-white shadow-[0_-1px_3px_rgba(0,0,0,0.05)]">
        <div className="flex gap-3">
          <Button 
            ...
            className="flex-1 rounded-full h-12 bg-slate-900 hover:bg-slate-800 text-white shadow-lg font-semibold"
          >
