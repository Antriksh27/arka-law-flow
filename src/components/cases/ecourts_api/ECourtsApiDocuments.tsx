...
interface ECourtsApiDocumentsProps {
  caseId: string;
}

export const ECourtsApiDocuments: React.FC<ECourtsApiDocumentsProps> = ({ caseId }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {isError && (
        <div className="text-sm text-destructive">Failed to fetch from eCourts API.</div>
      )}
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-10 text-muted-foreground">
          <Loader2 className="w-5 h-5 mr-2 animate-spin" /> Loading from eCourts…
        </div>
      ) : documents.length === 0 ? (
...
export default ECourtsApiDocuments;
