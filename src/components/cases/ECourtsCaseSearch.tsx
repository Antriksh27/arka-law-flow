...
import { resolveEcourtsSearchType, normalizeEcourtsCnr } from '@/lib/ecourtsSearchType';

interface ECourtsCaseSearchProps {
  caseId?: string;
  initialCnr?: string;
...
const isSearchTypeLocked = (cnr: string): boolean => {
  const normalized = normalizeEcourtsCnr(cnr);
  return normalized.startsWith('GJHC') || normalized.includes('GJHC');
};

export const ECourtsCaseSearch: React.FC<ECourtsCaseSearchProps> = ({
  caseId,
  initialCnr = '',
  onCaseDataFetched,
}) => {
  ...
  useEffect(() => {
    ...
    if (normalized.length >= 4) {
      const resolved = resolveEcourtsSearchType({ cnr });
  ...
  const searchCaseMutation = useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      console.log('📦 eCourts API Response:', JSON.stringify(data, null, 2));
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-semibold text-foreground">eCourts Case Search</h3>
              <p className="text-xs text-muted-foreground">Search for case details using eCourts API</p>
            </div>
          </div>
        </div>
