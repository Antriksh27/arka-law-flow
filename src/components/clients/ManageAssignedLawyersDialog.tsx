...
export const ManageAssignedLawyersDialog: React.FC<ManageAssignedLawyersDialogProps> = ({
  ...
  const {
    ...
    queryFn: async () => {
      ...
      } = await supabase.from('client_lawyer_assignments').select('id, lawyer_id, assigned_at').eq('client_id', clientId).eq('firm_id', firmId);
  ...
  const addLawyerMutation = useMutation({
    mutationFn: async (lawyerId: string) => {
      ...
      const {
        error
      } = await supabase.from('client_lawyer_assignments').insert({
        client_id: clientId,
        lawyer_id: lawyerId,
        firm_id: firmId,
        assigned_by: user?.id
  ...
  const removeLawyerMutation = useMutation({
    mutationFn: async (assignmentId: string) => {
      const {
        error
      } = await supabase.from('client_lawyer_assignments').delete().eq('id', assignmentId);
      if (error) throw error;
    },
    onSuccess: () => {
      toast({
  ...
  const handleAddLawyer = () => {
    ...
    console.log('Assign dialog: adding lawyer', {
      selectedLawyerId,
      clientId
    });
      ...
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6 pt-4">
          ...
          <div className="space-y-3">
            ...
            {assignedLawyers.length > 0 ? <div className="space-y-2">
                {assignedLawyers.map(assignment => <div key={assignment.assignmentId} className="flex items-center justify-between p-3 bg-blue-50 rounded-lg border border-blue-200">
                    ...
                    {canEdit && <Button size="sm" variant="outline" onClick={() => handleRemoveLawyer(assignment.assignmentId)} disabled={removeLawyerMutation.isPending} className="text-red-600 hover:text-red-700 hover:bg-red-50">
                        <Trash2 className="w-4 h-4 mr-1" />
                        Remove
                      </Button>}
                  </div>)}
              </div> : <div className="p-4 text-center text-gray-500 bg-gray-50 rounded-lg border-2 border-dashed border-gray-200">
                No lawyers currently assigned
              </div>}
          </div>

          {/* Add New Lawyer */}
          {canEdit && availableLawyers.length > 0 && <div className="space-y-3">
              <h4 className="text-sm font-medium text-gray-900">
                Add Lawyer
              </h4>
              ...
              setSelectedLawyerId(v);
                ...
                <Button type="button" onClick={handleAddLawyer} disabled={!selectedLawyerId || addLawyerMutation.isPending} className="shrink-0">
          ...
          {!canEdit && <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg">
          ...
          {canEdit && availableLawyers.length === 0 && assignedLawyers.length > 0 && <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
          ...
          {/* No Lawyers in Firm */}
          {canEdit && availableLawyers.length === 0 && assignedLawyers.length === 0 && <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg">
