...
export const HearingsCalendarView: React.FC<HearingsCalendarViewProps> = ({
  filters
}) => {
  const {
    openDialog
  ...
  const {
    ...
    queryFn: async () => {
      ...
      // Apply date range filter or default to show all hearings (past and future)
  ...
  const events: CalendarEvent[] = useMemo(() => {
    if (!hearings) return [];
    return hearings.filter(hearing => hearing.hearing_date).map(hearing => {
      // Use actual hearing_time if available, otherwise default to start of day
      const startTime = hearing.hearing_time ? parseISO(`${hearing.hearing_date}T${hearing.hearing_time}`) : parseISO(hearing.hearing_date);
  ...
  const handleSelectSlot = (slotInfo: {
    ...
    openDialog(<DayHearingsDialog selectedDate={selectedDate} hearings={dayHearings} onClose={() => {}} />);
  };
  ...
  const handleNextHearing = () => {
    ...
    const nextHearing = hearings.map(h => parseISO(h.hearing_date)).filter(d => d >= todayStart).sort((a, b) => a.getTime() - b.getTime())[0];
    if (nextHearing) {
      setDate(nextHearing);
    }
  };
  ...
  return <div className="bg-white rounded-2xl shadow-sm border border-[#E5E7EB] p-6">
      
      
      <div className="h-[700px]">
        <Calendar localizer={localizer} events={events} startAccessor="start" endAccessor="end" style={{
        height: '100%'
      }} onSelectEvent={handleSelectEvent} onSelectSlot={handleSelectSlot} selectable eventPropGetter={eventStyleGetter} view={view} onView={setView} date={date} onNavigate={setDate} views={['month', 'week', 'day']} popup />
      </div>
    </div>;
};