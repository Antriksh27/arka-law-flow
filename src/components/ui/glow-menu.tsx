  ...
  ({ className, items, activeItem, onItemClick }, ref) => {
    ...
    return (
      <motion.nav
        ref={ref}
        className={cn(
          "w-full px-6 py-2 rounded-full bg-background border border-border shadow-sm relative overflow-hidden",
          className,
        )}
        initial="initial"
        whileHover="hover"
            ...
            return (
              <motion.li key={item.label} className="relative flex-shrink-0">
                <button
                  ...
                  className={cn(
                    "flex items-center gap-2 px-5 py-2 rounded-full transition-all duration-200",
                    isActive 
