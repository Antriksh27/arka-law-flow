...
import {
  ChevronLeft, // Changed from ChevronLeftIcon to match common lucide import
  ChevronRight, // Changed from ChevronRightIcon
  PlusCircle, // Changed from PlusCircleIcon
...
export function FullScreenCalendar({
  ...
  const [currentMonthString, setCurrentMonthString] = React.useState(
    initialMonth || format(today, "MMM-yyyy"),
  );
  ...
  return (
    <div className="flex flex-1 flex-col bg-white border border-borderGray rounded-2xl shadow-sm">
      {/* Calendar Header */}
              ...
              !isDesktop ? (
                <button
                  ...
                  className={cn(
                    ...
                    isEqual(day, selectedDay) && "bg-primary/10", // Adjusted selected style
                    !isSameMonth(day, firstDayCurrentMonth) && "bg-accent/30 text-muted-foreground",
                ...
                >
                  <time
                    ...
                    className={cn(
                      ...
                      isToday(day) && !isEqual(day,selectedDay) && "border border-primary"
                    )}
                  >
                    {format(day, "d")}
                  </time>
                  {data.filter((date) => isSameDay(date.day, day)).length > 0 && (
                    <div className="mt-auto flex flex-wrap-reverse -mx-0.5">
                      {data
                        .filter((date) => isSameDay(date.day, day))[0]
                        .events.slice(0,3) // Show max 3 dots
                        .map((event) => (
                          <span
                            key={event.id}
                ...
                <div
                  ...
                  onClick={() => handleDayClick(day)}
                  className={cn(
                    "relative flex flex-col border-b border-r p-1 pt-0 hover:bg-muted focus:z-10 cursor-pointer min-h-[100px]", // Ensure min height
                ...
                >
                  <header className="flex items-center justify-end p-1.5">
                    <button
                      ...
                      className={cn(
                        ...
                        isEqual(day, selectedDay) && "bg-primary text-primary-foreground",
                        isToday(day) && !isEqual(day,selectedDay) && "font-semibold border border-primary text-primary",
                        !isToday(day) && !isEqual(day,selectedDay) && isSameMonth(day, firstDayCurrentMonth) && "text-foreground",
                  ...
                  <div className="flex-1 px-1.5 pb-1.5 space-y-0.5 overflow-y-auto">
                    {data
                      ...
                      .flatMap(d => d.events) // Get all events for the day
